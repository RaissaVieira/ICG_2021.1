...
material.vertexShader = `
    ...
    void main() {
        ...
        // 'V_cam' : vetor normalizado que aponta para a câmera.
        // Calculado a partir da posição do vértice e da posição da câmera.
        
        V_cam = normalize(vec3(position - vec3(0,0,25)));
...
material.fragmentShader = `
    ...
    void main() {
        ...
        vec3 N_cam_spc = normalize(vec3(N_cam_spc));
        vec3 L_cam_spc = normalize(vec3(L_cam_spc));
        vec3 R_cam_spc = normalize(vec3(R_cam_spc));
    
        vec3 V_cam = normalize(vec3(V_cam));

        // Equação descreve o modelo de Phong:

        vec3 term_eviroment = Ia * k_a;
        vec3 term_diffuse = Ip_diffuse_color * k_d * max(0.0, dot(N_cam_spc, L_cam_spc));
        vec3 term_specular = Ip_diffuse_color * k_s * pow(max(0.0, dot(R_cam_spc, V_cam)), 16.0);
        
        vec3 I_total = term_eviroment + term_diffuse + term_specular;
    
        // 'gl_FragColor' : variável de sistema que conterá a cor final do fragmento calculada pelo Fragment Shader.
        
        gl_FragColor = vec4(I_total, 0.0); 
    }
    `;
